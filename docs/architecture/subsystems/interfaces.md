...
may also support external client generation. The canonical first-party web
projection is `apps/web/src/lib/generated/api-contract.ts`, generated by
the interface-owned entrypoint `just contract-generate`.

`just contract-generate` always builds and runs the canonical Rust OpenAPI
binary (`tanren-api-openapi`) with locked dependencies before
`openapi-typescript` is invoked. `just web-contract-generate` delegates to
`just contract-generate` so the web package does not own generation logic
independently. `just check-web-contract-sync` is the drift check and fails
if the checked-in projection does not match canonical Rust generation output.
