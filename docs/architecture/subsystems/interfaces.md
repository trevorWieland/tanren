...
`just web-contract-generate` always builds and runs the canonical Rust OpenAPI
binary (`tanren-api-openapi`) with locked dependencies before
`openapi-typescript` is invoked. `just check-web-contract-sync` is the drift
check and fails if the checked-in projection does not match canonical Rust
generation output.
