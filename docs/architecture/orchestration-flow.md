...
Authoritative spec for the Tanren 2.0 methodology orchestration state
machine. Defines phases, transitions, failure routing, escalation,
monotonicity invariants, and cross-spec concerns.
...
[adherence.md](adherence.md), [phase-taxonomy.md](phase-taxonomy.md).
...
These commands live under `commands/project/` and do NOT participate
in the typed state machine. They declare their own autonomy:

- `sync-roadmap` — autonomous once consuming real spec-completion state
- `triage-audits` — interactive batch backlog curation
- `discover-standards` — interactive standard authoring
- `index-standards` — interactive standards-index maintenance
- `inject-standards` — interactive standards context injection
- `plan-product` — interactive product-doc authoring

### 1.3 Interactive surface

Exactly three phases pause for a human:
- `shape-spec`
- `walk-spec`
- `resolve-blockers`

All other spec-loop phases are autonomous. The escalation ladder
...
on `SpecFrontmatter`). Lane 0.5 emits the typed events; the resolution
...
Lane 0.5's orchestration flow composes with Lane 0.4's dispatch CRUD:
...
- Design rationale and decisions: [../rewrite/tasks/LANE-0.5-DESIGN-NOTES.md](../rewrite/tasks/LANE-0.5-DESIGN-NOTES.md)
