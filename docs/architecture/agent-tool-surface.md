...
- SDK: [`rmcp`](https://crates.io/crates/rmcp) from
  ...
  `transport-io`, `macros`.
- Runtime: tokio.
- Transport: stdio only. SSE/HTTP transports are deferred.
...
| `reset_task_guards(task_id, reason, idempotency_key?)` | `task.complete` | Clears all guard flags on an implemented task before investigate-driven remediation retries. |
