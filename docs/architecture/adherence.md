...
## 2. Relation to standards governance
...
execution. Standards discovery, indexing, and injection are project
commands outside the code/spec orchestration loop; they maintain the
standards corpus and agent context without mutating the active spec
task list.
...
- **Guard:** `SpecAdherent` (spec-level analog; the required spec-guard set
  follows the same model as task guards)
...
There is no separate `adherence.md` evidence doc; later roadmap phases may
add one if per-standard summary reports become useful.
