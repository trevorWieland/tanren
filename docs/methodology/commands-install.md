...
`tanren-cli install` renders Tanren's embedded command catalog into
per-agent-framework destinations, writes MCP server registration for each
framework, and installs the selected embedded standards profile.
...
```bash
tanren-cli install --profile rust-cargo  # first bootstrap in a fresh repo
tanren-cli install             # reinstall from committed tanren.yml
...
| `preserve_existing` | Never overwrite; create missing only | Standards profiles |
...
Command workflow content is owned by Tanren's embedded distribution assets.
Standards files are preserve-existing and remain user-owned after first
install.
