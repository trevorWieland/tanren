...
use clap::Args;
use serde::Serialize;
use tanren_app_services::methodology::config::{EnvironmentProfile, InstallFormat, MergePolicy};
...
use tanren_app_services::methodology::{MethodologyError, RequiredGuard, readonly_artifact_banner};

use super::install_bootstrap::{
    InstallLoadError, load_or_bootstrap_config, parse_agent_targets, resolve_config_path,
    resolve_install_paths, resolve_repo_root, validate_methodology_config,
};
...
pub(crate) struct InstallArgs {
    ...
    /// Path to `tanren.yml`. Relative paths resolve under
    /// `--repo-root`. Defaults to `tanren.yml`.
    #[arg(long, default_value = "tanren.yml")]
    pub config: PathBuf,

    /// Standards profile to install during first bootstrap. Existing
    /// repos must record this in `methodology.standards.profile`.
    #[arg(long)]
    pub profile: Option<String>,

    /// Agent integrations to install during first bootstrap
    /// (comma-separated). Defaults to all supported agents. Existing
    /// repos are controlled by `tanren.yml`.
    #[arg(long, value_delimiter = ',')]
    pub agents: Vec<String>,

    /// Restrict install to the given target formats (comma-separated).
    /// Accepted values:
    /// `claude-code`, `codex-skills`, `opencode`, `standards-profile`,
...
fn parse_target_filter(raw: &[String]) -> Result<Vec<InstallFormat>, String> {
    ...
    for s in raw {
        let fmt = match s.trim() {
            ...
            "standards-profile" => InstallFormat::StandardsProfile,
...
pub(crate) fn run(args: &InstallArgs) -> u8 {
    ...
    let plan = match build_install_plan(args, &target_filter) {
        Ok(plan) => plan,
...
fn build_install_plan(
    args: &InstallArgs,
    target_filter: &[InstallFormat],
) -> Result<InstallPlan, u8> {
    let requested_agents = match parse_agent_targets(&args.agents) {
        Ok(v) => v,
        Err(e) => return Err(fail_validation(&e)),
    };

    let repo_root = match resolve_repo_root(&args.repo_root) {
        Ok(path) => path,
        Err(e) => return Err(fail_io(&e)),
    };
    if let Err(e) = std::env::set_current_dir(&repo_root) {
        return Err(fail_io(&format!(
            "entering repo root {}: {e}",
            repo_root.display()
        )));
    }
    let config_path = resolve_config_path(&repo_root, &args.config);

    let loaded = match load_or_bootstrap_config(
        &config_path,
        ...
        Ok(loaded) => loaded,
        Err(InstallLoadError::Config(msg)) => return Err(fail_cfg(&msg)),
        Err(InstallLoadError::Validation(msg)) => return Err(fail_validation(&msg)),
    };
    let cfg = loaded.cfg;
    let mut methodology = cfg.methodology.clone();
    resolve_install_paths(&mut methodology, &repo_root);
    if let Err(e) = validate_methodology_config(&methodology) {
        return Err(fail_validation(&e));
    }
    ...
    let pillars =
        match tanren_app_services::methodology::rubric_registry::effective_pillars_for_runtime(
            ...
            Ok(pillars) => pillars,
            Err(err) => return Err(fail_validation(&err)),
        };
    ...
    let mut plan = match plan_install_from_root(&methodology, &context) {
        Ok(p) => p,
        Err(MethodologyError::Validation(e)) => return Err(fail_validation(&e)),
        Err(e) => return Err(fail_render(&e.to_string())),
    };
    if let Some(config_bytes) = loaded.generated_config_bytes {
        plan.writes.insert(
            0,
            tanren_app_services::methodology::installer::PlannedWrite {
                dest: config_path,
                bytes: config_bytes,
                merge_policy: MergePolicy::PreserveExisting,
                format: InstallFormat::TanrenConfig,
            },
        );
    }
    ...
    if !target_filter.is_empty() {
        plan.writes.retain(|w| {
            w.format == InstallFormat::TanrenConfig || target_filter.contains(&w.format)
        });
    }
