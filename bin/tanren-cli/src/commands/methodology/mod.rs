...
use tanren_app_services::methodology::{
    ...
    default_scope_for_phase, enter_mutation_session, finalize_mutation_session,
    parse_scope_env_for_phase,
};
use uuid::Uuid;

use self::capabilities::{PhaseCapabilitiesArgs, render_phase_capabilities};
...
pub(crate) enum MethodologyCommand {
    ...
    /// Generic check records.
    #[command(subcommand)]
    Check(check::CheckCommand),
    /// Durable investigation records.
    #[command(subcommand)]
    Investigation(investigation::InvestigationCommand),
    /// Rubric scoring.
    #[command(subcommand)]
...
pub(crate) async fn dispatch(
    ...
    let mut code = match command {
        ...
        MethodologyCommand::Check(c) => check::run(service, &scope, &phase, c).await,
        MethodologyCommand::Investigation(c) => {
            investigation::run(service, &scope, &phase, c).await
        }
...
fn is_mutation_command(command: &MethodologyCommand) -> bool {
    !matches!(
        ...
        MethodologyCommand::Standard(standard::StandardCommand::List(_))
            | MethodologyCommand::Finding(finding::FindingCommand::List(_))
            | MethodologyCommand::Investigation(investigation::InvestigationCommand::ListAttempts(
                _
            ),)
