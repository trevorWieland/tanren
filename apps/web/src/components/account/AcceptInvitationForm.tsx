...
export interface AcceptInvitationFormProps {
  token: string;
  onSuccess?: ((result: AcceptInvitationResult) => void) | undefined;
}
...
export function AcceptInvitationForm({
  token,
  onSuccess,
}: AcceptInvitationFormProps): ReactNode {
  const baseId = useId();
  ...
  function onSubmit(event: FormEvent<HTMLFormElement>): void {
    ...
    const parsed = v.safeParse(AcceptInvitationInput, {
      email,
      password,
      display_name: displayName,
    });
    if (!parsed.success) {
      setErrorMessage(m.acceptInvitation_required());
      return;
    }
    startTransition(async () => {
      try {
        const result = await acceptInvitation(token, parsed.output);
        onSuccess?.(result);
      } catch (cause: unknown) {
        if (cause instanceof AccountRequestError) {
          ...
          setErrorMessage(m.acceptInvitation_failed());
        }
      }
    });
  }
  ...
  return (
    <form
      onSubmit={onSubmit}
      noValidate
      aria-label={m.acceptInvitation_formLabel()}
      className="flex w-full max-w-md flex-col gap-4"
    >
      <div className="flex flex-col gap-1">
        <label htmlFor={emailId} className="text-sm font-medium">
          {m.acceptInvitation_email()}
        </label>
        <input
          id={emailId}
          name="email"
      ...
      <div className="flex flex-col gap-1">
        <label htmlFor={displayNameId} className="text-sm font-medium">
          {m.acceptInvitation_displayName()}
        </label>
        <input
          id={displayNameId}
          name="display_name"
      ...
      <button type="submit" disabled={pending} className={buttonClass}>
        {pending
          ? m.acceptInvitation_submitting()
          : m.acceptInvitation_submit()}
      </button>
      {errorActive && (
        <p
          id={errorId}
