...
export interface SignInFormProps {
  onSuccess?: ((result: SignInResult) => void) | undefined;
}
...
export function SignInForm({ onSuccess }: SignInFormProps): ReactNode {
  const baseId = useId();
  ...
  function onSubmit(event: FormEvent<HTMLFormElement>): void {
    ...
    const parsed = v.safeParse(SignInInput, { email, password });
    if (!parsed.success) {
      setErrorMessage(m.signIn_required());
      return;
    }
    startTransition(async () => {
      try {
        const result = await signIn(parsed.output);
        onSuccess?.(result);
      } catch (cause: unknown) {
        if (cause instanceof AccountRequestError) {
          ...
          setErrorMessage(m.signIn_failed());
        }
      }
    });
  }
  ...
  return (
    ...
    >
      ...
      <button type="submit" disabled={pending} className={buttonClass}>
        {pending ? m.signIn_submitting() : m.signIn_submit()}
      </button>
      {errorActive && (
        <p
          id={errorId}
