...
// Sources: crates/tanren-contract/src/role.rs, crates/tanren-identity-policy/src/role.rs, crates/tanren-api-app/src/routes.rs, crates/tanren-api-app/src/routes_role.rs.
...
export type PermissionGrantSource =
  | { source: "role_template"; role_id: RoleId }
  | { source: "direct_assignment" };

export interface PermissionGrantRevocation {
...
export interface PermissionGrantView {
  ...
  revocation: PermissionGrantRevocation | null;
  granted_at: string;
}
...
function parsePermissionGrantSource(
  value: unknown,
  context: string,
): PermissionGrantSource {
  const data = expectRecord(value, context);
  const variant = parseString(data["source"], `${context}.source`);
  switch (variant) {
    case "role_template":
      return {
        source: variant,
    ...
    default:
      throw new Error(
        `${context}.source must be one of role_template, direct_assignment`,
      );
  }
}
...
function parsePermissionGrantRevocation(
  value: unknown,
  context: string,
): PermissionGrantRevocation {
  const data = expectRecord(value, context);
  return {
    revoked_by: parsePrincipalRef(data["revoked_by"], `${context}.revoked_by`),
    revoked_at: parseString(data["revoked_at"], `${context}.revoked_at`),
  };
}
...
function parsePermissionGrantView(
  ...
  return {
    ...
    source: parsePermissionGrantSource(data["source"], `${context}.source`),
    revocation: parseOptional(
      data["revocation"],
      `${context}.revocation`,
