/**
 * This file was auto-generated by openapi-typescript.
 * Do not make direct changes to the file.
 */

export interface paths {
  "/accounts": {
    parameters: {
      query?: never;
      header?: never;
      path?: never;
      cookie?: never;
    };
    get?: never;
    put?: never;
    /**
     * Self-signup: create a new personal account and mint a cookie-bound
     *     session.
     */
    post: operations["sign_up_route"];
    delete?: never;
    options?: never;
    head?: never;
    patch?: never;
    trace?: never;
  };
  "/accounts/{account_id}/user-credentials": {
    parameters: {
      query?: never;
      header?: never;
      path?: never;
      cookie?: never;
    };
    get: operations["list_user_credentials_route"];
    put?: never;
    post: operations["add_user_credential_route"];
    delete?: never;
    options?: never;
    head?: never;
    patch?: never;
    trace?: never;
  };
  "/accounts/{account_id}/user-credentials/{item_id}": {
    parameters: {
      query?: never;
      header?: never;
      path?: never;
      cookie?: never;
    };
    get?: never;
    put: operations["update_user_credential_route"];
    post?: never;
    delete: operations["remove_user_credential_route"];
    options?: never;
    head?: never;
    patch?: never;
    trace?: never;
  };
  "/accounts/{account_id}/user-settings": {
    parameters: {
      query?: never;
      header?: never;
      path?: never;
      cookie?: never;
    };
    get: operations["list_user_settings_route"];
    put?: never;
    post: operations["upsert_user_setting_route"];
    delete?: never;
    options?: never;
    head?: never;
    patch?: never;
    trace?: never;
  };
  "/accounts/{account_id}/user-settings/{key}": {
    parameters: {
      query?: never;
      header?: never;
      path?: never;
      cookie?: never;
    };
    get?: never;
    put?: never;
    post?: never;
    delete: operations["remove_user_setting_route"];
    options?: never;
    head?: never;
    patch?: never;
    trace?: never;
  };
  "/configuration/account/capabilities": {
    parameters: {
      query?: never;
      header?: never;
      path?: never;
      cookie?: never;
    };
    get: operations["get_authenticated_user_configuration_capabilities_route"];
    put?: never;
    post?: never;
    delete?: never;
    options?: never;
    head?: never;
    patch?: never;
    trace?: never;
  };
  "/configuration/account/user-credentials": {
    parameters: {
      query?: never;
      header?: never;
      path?: never;
      cookie?: never;
    };
    get: operations["list_authenticated_user_credentials_route"];
    put?: never;
    post: operations["add_authenticated_user_credential_route"];
    delete?: never;
    options?: never;
    head?: never;
    patch?: never;
    trace?: never;
  };
  "/configuration/account/user-credentials/{item_id}": {
    parameters: {
      query?: never;
      header?: never;
      path?: never;
      cookie?: never;
    };
    get?: never;
    put: operations["update_authenticated_user_credential_route"];
    post?: never;
    delete: operations["remove_authenticated_user_credential_route"];
    options?: never;
    head?: never;
    patch?: never;
    trace?: never;
  };
  "/configuration/account/user-settings": {
    parameters: {
      query?: never;
      header?: never;
      path?: never;
      cookie?: never;
    };
    get: operations["list_authenticated_user_settings_route"];
    put?: never;
    post: operations["upsert_authenticated_user_setting_route"];
    delete?: never;
    options?: never;
    head?: never;
    patch?: never;
    trace?: never;
  };
  "/configuration/account/user-settings/{key}": {
    parameters: {
      query?: never;
      header?: never;
      path?: never;
      cookie?: never;
    };
    get?: never;
    put?: never;
    post?: never;
    delete: operations["remove_authenticated_user_setting_route"];
    options?: never;
    head?: never;
    patch?: never;
    trace?: never;
  };
  "/health": {
    parameters: {
      query?: never;
      header?: never;
      path?: never;
      cookie?: never;
    };
    /** Liveness probe. */
    get: operations["health_route"];
    put?: never;
    post?: never;
    delete?: never;
    options?: never;
    head?: never;
    patch?: never;
    trace?: never;
  };
  "/invitations/{token}/accept": {
    parameters: {
      query?: never;
      header?: never;
      path?: never;
      cookie?: never;
    };
    get?: never;
    put?: never;
    /** Accept an organization invitation and mint a cookie-bound session. */
    post: operations["accept_invitation_route"];
    delete?: never;
    options?: never;
    head?: never;
    patch?: never;
    trace?: never;
  };
  "/sessions": {
    parameters: {
      query?: never;
      header?: never;
      path?: never;
      cookie?: never;
    };
    get?: never;
    put?: never;
    /** Sign-in: mint a cookie-bound session for an existing account. */
    post: operations["sign_in_route"];
    delete?: never;
    options?: never;
    head?: never;
    patch?: never;
    trace?: never;
  };
  "/sessions/revoke": {
    parameters: {
      query?: never;
      header?: never;
      path?: never;
      cookie?: never;
    };
    get?: never;
    put?: never;
    /**
     * Revoke (sign out) the current session. Clears the cookie via
     *     `Session::flush` and returns 204.
     */
    post: operations["revoke_route"];
    delete?: never;
    options?: never;
    head?: never;
    patch?: never;
    trace?: never;
  };
}
export type webhooks = Record<string, never>;
export interface components {
  schemas: {
    /**
     * @description Path body for `POST /invitations/{token}/accept`. Splits the password
     *     into a `String` here (then re-wraps as `SecretString` before handing
     *     off to app-services) so utoipa can document the schema; the secret
     *     stays in memory only for the lifetime of this function.
     */
    AcceptInvitationBody: {
      /** @description Display name. */
      display_name: string;
      /** @description Email the invitee chose. */
      email: components["schemas"]["Email"];
      /**
       * Format: password
       * @description Plaintext password.
       */
      password: string;
    };
    /** @description Cookie-transport projection of an invitation-acceptance response. */
    AcceptInvitationResponseCookie: {
      /** @description View of the newly created account. */
      account: components["schemas"]["AccountView"];
      /** @description Organization the new account joined. */
      joined_org: components["schemas"]["OrgId"];
      /** @description Cookie-projected session envelope. */
      session: components["schemas"]["SessionEnvelope"];
    };
    /** @description Shared `{code, summary}` failure body. */
    AccountFailureBody: {
      /** @description Stable error code from the closed taxonomy. */
      code: string;
      /** @description Human-readable summary. */
      summary: string;
    };
    /**
     * Format: uuid
     * @description Stable identifier for a Tanren account. `UUIDv7` — sortable + unique.
     */
    AccountId: string;
    /** @description External-facing view of a Tanren account. */
    AccountView: {
      /** @description Display name. */
      display_name: string;
      /** @description Stable account id. */
      id: components["schemas"]["AccountId"];
      /** @description User-facing identifier (email). */
      identifier: components["schemas"]["Identifier"];
      org?: null | components["schemas"]["OrgId"];
    };
    AuthenticatedCreateUserCredentialRequest: {
      kind: components["schemas"]["UserCredentialKind"];
      /** Format: password */
      value: string;
    };
    AuthenticatedUpdateUserCredentialRequest: {
      /** Format: password */
      value: string;
    };
    AuthenticatedUpsertUserSettingRequest: {
      key: components["schemas"]["UserSettingKey"];
      value: components["schemas"]["UserSettingValue"];
    };
    /** @description Capability map for authenticated account-configuration operations. */
    ConfigurationCapabilitiesView: {
      /** @description Capability set for setting reads and writes. */
      settings: components["schemas"]["SettingCapabilitiesView"];
      /** @description Capability set for user-owned secret-item metadata reads and writes. */
      user_items: components["schemas"]["CredentialCapabilitiesView"];
    };
    /** @description Typed validation failures for user-tier setting and credential payloads. */
    ConfigurationValidationFailure:
      | {
          /** @description Value kind expected by the key. */
          expected: components["schemas"]["UserSettingValueKind"];
          /** @description Key being written. */
          key: components["schemas"]["UserSettingKey"];
          /** @description Value kind supplied by the caller. */
          provided: components["schemas"]["UserSettingValueKind"];
          /** @enum {string} */
          reason: "setting_type_mismatch";
        }
      | {
          /** @enum {string} */
          reason: "editor_empty";
        }
      | {
          /** @description Provided byte length. */
          actual_bytes: number;
          /** @description Maximum byte length accepted by the contract. */
          max_bytes: number;
          /** @enum {string} */
          reason: "editor_too_long";
        }
      | {
          /** @enum {string} */
          reason: "value_empty";
        }
      | {
          /** @description Provided byte length. */
          actual_bytes: number;
          /** @description Maximum byte length accepted by the contract. */
          max_bytes: number;
          /** @enum {string} */
          reason: "value_too_long";
        };
    /** @description Create request for a user-owned credential. */
    CreateUserCredentialRequest: {
      /** @description Credential kind. */
      kind: components["schemas"]["UserCredentialKind"];
      /** @description Owning scope. */
      owner_scope: components["schemas"]["OwnerScope"];
      /**
       * Format: password
       * @description Fresh secret value.
       */
      value: string;
    };
    /** @description Create response for a user-owned credential. */
    CreateUserCredentialResponse: {
      /** @description Redacted metadata view. */
      item: components["schemas"]["UserCredentialView"];
    };
    /** @description Capability set for user-owned credential operations. */
    CredentialCapabilitiesView: {
      /** @description Allowed secret-item actions for the authenticated actor. */
      allowed_actions: components["schemas"]["CredentialCapabilityAction"][];
    };
    /**
     * @description Per-action capabilities for user-owned secret items.
     * @enum {string}
     */
    CredentialCapabilityAction: "read" | "create" | "update" | "delete";
    /**
     * Format: email
     * @description Validated email address. Constructed via [`Email::parse`] which:
     *     trims surrounding whitespace, validates against RFC 5322 syntax via
     *     the [`email_validator_rfc5322`] crate (RFC 5321 length limits +
     *     quoted local parts), additionally requires a TLD-style domain (no
     *     dotless or IP-literal domains), and canonicalises to lower-case so
     *     case variants of the same address compare equal.
     *
     *     # Wire-input contract
     *
     *     `Email` does NOT derive `Deserialize` — the custom impl below routes
     *     every wire input through [`parse`](Self::parse). Without this,
     *     `#[serde(transparent)]` would let HTTP/MCP/CLI requests carry
     *     untrimmed/un-lowercased/RFC-invalid addresses, which would persist
     *     verbatim via `Identifier::from_email` and let two case variants of
     *     the same logical email register as separate accounts. Codex P1
     *     review on PR #133.
     *
     *     Validation invariants are exercised end-to-end by the @api / @web
     *     scenarios in `tests/bdd/features/B-0043-create-account.feature` —
     *     case-variant rejection and malformed-email rejection both run
     *     through the live wire surface, not through Rust unit tests.
     */
    Email: string;
    /** @description Response for authenticated configuration capability discovery. */
    GetAuthenticatedUserConfigurationCapabilitiesResponse: {
      /** @description Configuration contract version for this response shape. */
      version: components["schemas"]["ConfigurationVersion"];
      /** @description Capabilities granted to the authenticated actor. */
      capabilities: components["schemas"]["ConfigurationCapabilitiesView"];
    };
    /** @description Version discriminator for configuration capability responses. */
    ConfigurationVersion: {
      /**
       * Format: int32
       * @description Numeric value.
       */
      value: number;
    };
    /** @description Liveness response. */
    HealthResponse: {
      /**
       * Format: int32
       * @description Wire-contract version.
       */
      contract_version: number;
      /** @description Static "ok" string. */
      status: string;
      /** @description Build-time package version. */
      version: string;
    };
    /**
     * @description User-facing identifier for an account. R-0001's chosen mechanism is
     *     identifier+password where the identifier is the canonical email; the
     *     type wraps the raw string so future mechanisms can lift constraints
     *     in one place.
     *
     *     `Identifier` does NOT derive `Deserialize` — the custom impl below
     *     routes every wire input through [`parse`](Self::parse) so untrimmed
     *     or differently-cased identifiers cannot bypass canonicalisation.
     *     Validation invariants are exercised end-to-end by the @api / @web
     *     scenarios in `tests/bdd/features/B-0043-create-account.feature`
     *     (case-variant rejection, malformed-input rejection); per the
     *     BDD-only test surface policy there are no Rust unit or doc-tests
     *     for these rules.
     */
    Identifier: string;
    /** @description List response for user-owned credentials. */
    ListUserCredentialsResponse: {
      /** @description Redacted metadata rows. */
      items: components["schemas"]["UserCredentialView"][];
    };
    /** @description List response for user-tier settings. */
    ListUserSettingsResponse: {
      /** @description Persisted setting views. */
      items: components["schemas"]["UserSettingView"][];
    };
    /**
     * Format: uuid
     * @description Stable identifier for a Tanren organization.
     */
    OrgId: string;
    /** @description Account-scoped owner shape for user-owned configuration and secrets. */
    OwnerScope: {
      /** @description Owning account id. */
      account_id: components["schemas"]["AccountId"];
      /** @enum {string} */
      scope: "user";
    };
    /** @description Remove response for a user-owned credential. */
    RemoveUserCredentialResponse: {
      /** @description Removed credential metadata from the pre-delete snapshot. */
      item: components["schemas"]["UserCredentialView"];
    };
    /** @description Remove response for a user-tier setting. */
    RemoveUserSettingResponse: {
      /** @description Removed setting view from the pre-delete snapshot. */
      setting: components["schemas"]["UserSettingView"];
    };
    /**
     * @description Transport-aware projection of a freshly minted session.
     *
     *     The `@web` and `@api` surfaces deliver session tokens via an
     *     `HttpOnly + Secure + SameSite=Strict` cookie set by the API; the body
     *     only exposes `account_id` + `expires_at` (`Cookie` variant). The
     *     `@cli`, `@mcp`, and `@tui` surfaces have no cookie jar — they receive
     *     the token in the response body (`Bearer` variant). Subsequent PRs map
     *     `SessionView` → `SessionEnvelope` per surface inside each binary
     *     (cookie session lands in PR 8). The discriminator is the transport,
     *     not the user.
     *
     *     See `docs/architecture/subsystems/interfaces.md` § "Canonical session,
     *     error, `OpenAPI`, and design-token decisions" and
     *     `profiles/rust-cargo/architecture/cookie-session.md`.
     */
    SessionEnvelope:
      | {
          /** @description Account this session is bound to. */
          account_id: components["schemas"]["AccountId"];
          /**
           * Format: date-time
           * @description Wall-clock time at which the session expires.
           */
          expires_at: string;
          /** @enum {string} */
          transport: "cookie";
        }
      | {
          /** @description Account this session is bound to. */
          account_id: components["schemas"]["AccountId"];
          /**
           * Format: date-time
           * @description Wall-clock time at which the session expires.
           */
          expires_at: string;
          /** @description Opaque session token. */
          token: components["schemas"]["SessionToken"];
          /** @enum {string} */
          transport: "bearer";
        };
    SessionToken: string;
    /** @description Capability set for user-tier settings operations. */
    SettingCapabilitiesView: {
      /** @description Allowed settings actions for the authenticated actor. */
      allowed_actions: components["schemas"]["SettingCapabilityAction"][];
    };
    /**
     * @description Per-action capabilities for user-tier settings.
     * @enum {string}
     */
    SettingCapabilityAction: "read" | "create_or_update" | "delete";
    /** @description Sign-in request. */
    SignInRequest: {
      /** @description Email of the account being signed in to. */
      email: components["schemas"]["Email"];
      /**
       * Format: password
       * @description Plaintext password — verified against the stored hash.
       */
      password: string;
    };
    /** @description Cookie-transport projection of a sign-in response. */
    SignInResponseCookie: {
      /** @description View of the signed-in account. */
      account: components["schemas"]["AccountView"];
      /** @description Cookie-projected session envelope. */
      session: components["schemas"]["SessionEnvelope"];
    };
    /** @description Self-signup request. */
    SignUpRequest: {
      /** @description Human-readable display name for the new account. */
      display_name: string;
      /**
       * @description Email address that will own the new account. Lower-cased + trimmed
       *     during validation.
       */
      email: components["schemas"]["Email"];
      /**
       * Format: password
       * @description Plaintext password. Hashed by the handler before persistence.
       *     Wrapped in `SecretString` so accidental `Debug` / `Serialize`
       *     calls do not leak the credential.
       */
      password: string;
    };
    /**
     * @description Cookie-transport response shape for the api surface. Mirrors
     *     `SignUpResponse`/`SignInResponse`/`AcceptInvitationResponse` but
     *     projects the session into [`SessionEnvelope::Cookie`] (no token in
     *     body — it ships in the `Set-Cookie` header).
     */
    SignUpResponseCookie: {
      /** @description View of the freshly created account. */
      account: components["schemas"]["AccountView"];
      /** @description Cookie-projected session envelope. */
      session: components["schemas"]["SessionEnvelope"];
    };
    /**
     * @description Theme preference for user-tier configuration.
     * @enum {string}
     */
    ThemePreference: "system" | "light" | "dark";
    /** @description Update request for an existing user-owned credential. */
    UpdateUserCredentialRequest: {
      /**
       * Format: password
       * @description Replacement secret value.
       */
      value: string;
    };
    /** @description Update response for a user-owned credential. */
    UpdateUserCredentialResponse: {
      /** @description Redacted metadata view. */
      item: components["schemas"]["UserCredentialView"];
    };
    /** @description Upsert request for a user-tier setting. */
    UpsertUserSettingRequest: {
      /** @description Setting key. */
      key: components["schemas"]["UserSettingKey"];
      /** @description Typed value payload for the key. */
      value: components["schemas"]["UserSettingValue"];
    };
    /** @description Upsert response for a user-tier setting. */
    UpsertUserSettingResponse: {
      /** @description Persisted setting view. */
      setting: components["schemas"]["UserSettingView"];
    };
    /** @description Closed taxonomy of user-configuration and user-credential failures. */
    UserConfigurationFailureReason:
      | {
          /** @enum {string} */
          code: "validation_failed";
          /** @description Typed validation detail. */
          detail: components["schemas"]["ConfigurationValidationFailure"];
        }
      | {
          /** @enum {string} */
          code: "setting_not_found";
        }
      | {
          /** @enum {string} */
          code: "item_not_found";
        };
    /**
     * @description User-owned credential kinds covered by B-0048/B-0125's initial contract.
     * @enum {string}
     */
    UserCredentialKind: "provider_api_token" | "harness_api_token";
    /**
     * @description Lifecycle status for stored user-owned credentials.
     * @enum {string}
     */
    UserCredentialStatus: "pending" | "active" | "invalid";
    /** @description Redacted read/list view for a user-owned credential. */
    UserCredentialView: {
      /**
       * Format: date-time
       * @description Creation timestamp.
       */
      created_at: string;
      /** @description Stable metadata id. */
      id: string;
      /** @description Credential kind. */
      kind: components["schemas"]["UserCredentialKind"];
      /** @description Owning scope. */
      owner_scope: components["schemas"]["OwnerScope"];
      /** @description Lifecycle status. */
      status: components["schemas"]["UserCredentialStatus"];
      /**
       * Format: date-time
       * @description Last update timestamp.
       */
      updated_at: string;
    };
    /**
     * @description Closed set of user-tier setting keys covered by B-0048's initial contract.
     * @enum {string}
     */
    UserSettingKey: "theme" | "editor";
    /** @description Typed value payload for a user-tier setting. */
    UserSettingValue:
      | {
          /** @enum {string} */
          kind: "theme";
          /** @description Theme preference value. */
          value: components["schemas"]["ThemePreference"];
        }
      | {
          /** @enum {string} */
          kind: "editor";
          /** @description Editor command value (`code`, `vim`, `nvim`, ...). */
          value: string;
        };
    /**
     * @description Type discriminator for user setting values.
     * @enum {string}
     */
    UserSettingValueKind: "theme" | "editor";
    /** @description Read view for a user-tier setting. */
    UserSettingView: {
      /** @description Setting key. */
      key: components["schemas"]["UserSettingKey"];
      /**
       * Format: date-time
       * @description Last update timestamp.
       */
      updated_at: string;
      /** @description Typed setting value. */
      value: components["schemas"]["UserSettingValue"];
    };
  };
  responses: never;
  parameters: never;
  requestBodies: never;
  headers: never;
  pathItems: never;
}
export type $defs = Record<string, never>;
export interface operations {
  sign_up_route: {
    parameters: {
      query?: never;
      header?: never;
      path?: never;
      cookie?: never;
    };
    requestBody: {
      content: {
        "application/json": components["schemas"]["SignUpRequest"];
      };
    };
    responses: {
      /** @description Account created */
      201: {
        headers: {
          [name: string]: unknown;
        };
        content: {
          "application/json": components["schemas"]["SignUpResponseCookie"];
        };
      };
      /** @description validation_failed */
      400: {
        headers: {
          [name: string]: unknown;
        };
        content: {
          "application/json": components["schemas"]["AccountFailureBody"];
        };
      };
      /** @description invalid_credential */
      401: {
        headers: {
          [name: string]: unknown;
        };
        content: {
          "application/json": components["schemas"]["AccountFailureBody"];
        };
      };
      /** @description duplicate_identifier */
      409: {
        headers: {
          [name: string]: unknown;
        };
        content: {
          "application/json": components["schemas"]["AccountFailureBody"];
        };
      };
    };
  };
  list_user_credentials_route: {
    parameters: {
      query?: never;
      header?: never;
      path: {
        /** @description Target account id */
        account_id: components["schemas"]["AccountId"];
      };
      cookie?: never;
    };
    requestBody?: never;
    responses: {
      200: {
        headers: {
          [name: string]: unknown;
        };
        content: {
          "application/json": components["schemas"]["ListUserCredentialsResponse"];
        };
      };
      /** @description validation_failed */
      400: {
        headers: {
          [name: string]: unknown;
        };
        content: {
          "application/json": components["schemas"]["AccountFailureBody"];
        };
      };
      /** @description auth_required */
      401: {
        headers: {
          [name: string]: unknown;
        };
        content: {
          "application/json": components["schemas"]["AccountFailureBody"];
        };
      };
      /** @description item_not_found */
      404: {
        headers: {
          [name: string]: unknown;
        };
        content: {
          "application/json": components["schemas"]["AccountFailureBody"];
        };
      };
    };
  };
  add_user_credential_route: {
    parameters: {
      query?: never;
      header?: never;
      path: {
        /** @description Target account id */
        account_id: components["schemas"]["AccountId"];
      };
      cookie?: never;
    };
    requestBody: {
      content: {
        "application/json": components["schemas"]["CreateUserCredentialRequest"];
      };
    };
    responses: {
      201: {
        headers: {
          [name: string]: unknown;
        };
        content: {
          "application/json": components["schemas"]["CreateUserCredentialResponse"];
        };
      };
      /** @description validation_failed */
      400: {
        headers: {
          [name: string]: unknown;
        };
        content: {
          "application/json": components["schemas"]["AccountFailureBody"];
        };
      };
      /** @description auth_required */
      401: {
        headers: {
          [name: string]: unknown;
        };
        content: {
          "application/json": components["schemas"]["AccountFailureBody"];
        };
      };
      /** @description item_not_found */
      404: {
        headers: {
          [name: string]: unknown;
        };
        content: {
          "application/json": components["schemas"]["AccountFailureBody"];
        };
      };
    };
  };
  update_user_credential_route: {
    parameters: {
      query?: never;
      header?: never;
      path: {
        /** @description Target account id */
        account_id: components["schemas"]["AccountId"];
        /** @description Credential metadata id */
        item_id: string;
      };
      cookie?: never;
    };
    requestBody: {
      content: {
        "application/json": components["schemas"]["UpdateUserCredentialRequest"];
      };
    };
    responses: {
      200: {
        headers: {
          [name: string]: unknown;
        };
        content: {
          "application/json": components["schemas"]["UpdateUserCredentialResponse"];
        };
      };
      /** @description validation_failed */
      400: {
        headers: {
          [name: string]: unknown;
        };
        content: {
          "application/json": components["schemas"]["AccountFailureBody"];
        };
      };
      /** @description auth_required */
      401: {
        headers: {
          [name: string]: unknown;
        };
        content: {
          "application/json": components["schemas"]["AccountFailureBody"];
        };
      };
      /** @description item_not_found */
      404: {
        headers: {
          [name: string]: unknown;
        };
        content: {
          "application/json": components["schemas"]["AccountFailureBody"];
        };
      };
    };
  };
  remove_user_credential_route: {
    parameters: {
      query?: never;
      header?: never;
      path: {
        /** @description Target account id */
        account_id: components["schemas"]["AccountId"];
        /** @description Credential metadata id */
        item_id: string;
      };
      cookie?: never;
    };
    requestBody?: never;
    responses: {
      200: {
        headers: {
          [name: string]: unknown;
        };
        content: {
          "application/json": components["schemas"]["RemoveUserCredentialResponse"];
        };
      };
      /** @description validation_failed */
      400: {
        headers: {
          [name: string]: unknown;
        };
        content: {
          "application/json": components["schemas"]["AccountFailureBody"];
        };
      };
      /** @description auth_required */
      401: {
        headers: {
          [name: string]: unknown;
        };
        content: {
          "application/json": components["schemas"]["AccountFailureBody"];
        };
      };
      /** @description item_not_found */
      404: {
        headers: {
          [name: string]: unknown;
        };
        content: {
          "application/json": components["schemas"]["AccountFailureBody"];
        };
      };
    };
  };
  list_user_settings_route: {
    parameters: {
      query?: never;
      header?: never;
      path: {
        /** @description Target account id */
        account_id: components["schemas"]["AccountId"];
      };
      cookie?: never;
    };
    requestBody?: never;
    responses: {
      200: {
        headers: {
          [name: string]: unknown;
        };
        content: {
          "application/json": components["schemas"]["ListUserSettingsResponse"];
        };
      };
      /** @description validation_failed */
      400: {
        headers: {
          [name: string]: unknown;
        };
        content: {
          "application/json": components["schemas"]["AccountFailureBody"];
        };
      };
      /** @description auth_required */
      401: {
        headers: {
          [name: string]: unknown;
        };
        content: {
          "application/json": components["schemas"]["AccountFailureBody"];
        };
      };
      /** @description setting_not_found */
      404: {
        headers: {
          [name: string]: unknown;
        };
        content: {
          "application/json": components["schemas"]["AccountFailureBody"];
        };
      };
    };
  };
  upsert_user_setting_route: {
    parameters: {
      query?: never;
      header?: never;
      path: {
        /** @description Target account id */
        account_id: components["schemas"]["AccountId"];
      };
      cookie?: never;
    };
    requestBody: {
      content: {
        "application/json": components["schemas"]["UpsertUserSettingRequest"];
      };
    };
    responses: {
      200: {
        headers: {
          [name: string]: unknown;
        };
        content: {
          "application/json": components["schemas"]["UpsertUserSettingResponse"];
        };
      };
      /** @description validation_failed */
      400: {
        headers: {
          [name: string]: unknown;
        };
        content: {
          "application/json": components["schemas"]["AccountFailureBody"];
        };
      };
      /** @description auth_required */
      401: {
        headers: {
          [name: string]: unknown;
        };
        content: {
          "application/json": components["schemas"]["AccountFailureBody"];
        };
      };
      /** @description setting_not_found */
      404: {
        headers: {
          [name: string]: unknown;
        };
        content: {
          "application/json": components["schemas"]["AccountFailureBody"];
        };
      };
    };
  };
  remove_user_setting_route: {
    parameters: {
      query?: never;
      header?: never;
      path: {
        /** @description Target account id */
        account_id: components["schemas"]["AccountId"];
        /** @description User setting key to remove */
        key: components["schemas"]["UserSettingKey"];
      };
      cookie?: never;
    };
    requestBody?: never;
    responses: {
      200: {
        headers: {
          [name: string]: unknown;
        };
        content: {
          "application/json": components["schemas"]["RemoveUserSettingResponse"];
        };
      };
      /** @description validation_failed */
      400: {
        headers: {
          [name: string]: unknown;
        };
        content: {
          "application/json": components["schemas"]["AccountFailureBody"];
        };
      };
      /** @description auth_required */
      401: {
        headers: {
          [name: string]: unknown;
        };
        content: {
          "application/json": components["schemas"]["AccountFailureBody"];
        };
      };
      /** @description setting_not_found */
      404: {
        headers: {
          [name: string]: unknown;
        };
        content: {
          "application/json": components["schemas"]["AccountFailureBody"];
        };
      };
    };
  };
  get_authenticated_user_configuration_capabilities_route: {
    parameters: {
      query?: never;
      header?: never;
      path?: never;
      cookie?: never;
    };
    requestBody?: never;
    responses: {
      200: {
        headers: {
          [name: string]: unknown;
        };
        content: {
          "application/json": components["schemas"]["GetAuthenticatedUserConfigurationCapabilitiesResponse"];
        };
      };
      /** @description auth_required */
      401: {
        headers: {
          [name: string]: unknown;
        };
        content: {
          "application/json": components["schemas"]["AccountFailureBody"];
        };
      };
    };
  };
  list_authenticated_user_credentials_route: {
    parameters: {
      query?: never;
      header?: never;
      path?: never;
      cookie?: never;
    };
    requestBody?: never;
    responses: {
      200: {
        headers: {
          [name: string]: unknown;
        };
        content: {
          "application/json": components["schemas"]["ListUserCredentialsResponse"];
        };
      };
      /** @description auth_required */
      401: {
        headers: {
          [name: string]: unknown;
        };
        content: {
          "application/json": components["schemas"]["AccountFailureBody"];
        };
      };
      /** @description item_not_found */
      404: {
        headers: {
          [name: string]: unknown;
        };
        content: {
          "application/json": components["schemas"]["AccountFailureBody"];
        };
      };
    };
  };
  add_authenticated_user_credential_route: {
    parameters: {
      query?: never;
      header?: never;
      path?: never;
      cookie?: never;
    };
    requestBody: {
      content: {
        "application/json": components["schemas"]["AuthenticatedCreateUserCredentialRequest"];
      };
    };
    responses: {
      201: {
        headers: {
          [name: string]: unknown;
        };
        content: {
          "application/json": components["schemas"]["CreateUserCredentialResponse"];
        };
      };
      /** @description validation_failed */
      400: {
        headers: {
          [name: string]: unknown;
        };
        content: {
          "application/json": components["schemas"]["AccountFailureBody"];
        };
      };
      /** @description auth_required */
      401: {
        headers: {
          [name: string]: unknown;
        };
        content: {
          "application/json": components["schemas"]["AccountFailureBody"];
        };
      };
      /** @description item_not_found */
      404: {
        headers: {
          [name: string]: unknown;
        };
        content: {
          "application/json": components["schemas"]["AccountFailureBody"];
        };
      };
    };
  };
  update_authenticated_user_credential_route: {
    parameters: {
      query?: never;
      header?: never;
      path: {
        /** @description Credential metadata id */
        item_id: string;
      };
      cookie?: never;
    };
    requestBody: {
      content: {
        "application/json": components["schemas"]["AuthenticatedUpdateUserCredentialRequest"];
      };
    };
    responses: {
      200: {
        headers: {
          [name: string]: unknown;
        };
        content: {
          "application/json": components["schemas"]["UpdateUserCredentialResponse"];
        };
      };
      /** @description validation_failed */
      400: {
        headers: {
          [name: string]: unknown;
        };
        content: {
          "application/json": components["schemas"]["AccountFailureBody"];
        };
      };
      /** @description auth_required */
      401: {
        headers: {
          [name: string]: unknown;
        };
        content: {
          "application/json": components["schemas"]["AccountFailureBody"];
        };
      };
      /** @description item_not_found */
      404: {
        headers: {
          [name: string]: unknown;
        };
        content: {
          "application/json": components["schemas"]["AccountFailureBody"];
        };
      };
    };
  };
  remove_authenticated_user_credential_route: {
    parameters: {
      query?: never;
      header?: never;
      path: {
        /** @description Credential metadata id */
        item_id: string;
      };
      cookie?: never;
    };
    requestBody?: never;
    responses: {
      200: {
        headers: {
          [name: string]: unknown;
        };
        content: {
          "application/json": components["schemas"]["RemoveUserCredentialResponse"];
        };
      };
      /** @description auth_required */
      401: {
        headers: {
          [name: string]: unknown;
        };
        content: {
          "application/json": components["schemas"]["AccountFailureBody"];
        };
      };
      /** @description item_not_found */
      404: {
        headers: {
          [name: string]: unknown;
        };
        content: {
          "application/json": components["schemas"]["AccountFailureBody"];
        };
      };
    };
  };
  list_authenticated_user_settings_route: {
    parameters: {
      query?: never;
      header?: never;
      path?: never;
      cookie?: never;
    };
    requestBody?: never;
    responses: {
      200: {
        headers: {
          [name: string]: unknown;
        };
        content: {
          "application/json": components["schemas"]["ListUserSettingsResponse"];
        };
      };
      /** @description auth_required */
      401: {
        headers: {
          [name: string]: unknown;
        };
        content: {
          "application/json": components["schemas"]["AccountFailureBody"];
        };
      };
      /** @description setting_not_found */
      404: {
        headers: {
          [name: string]: unknown;
        };
        content: {
          "application/json": components["schemas"]["AccountFailureBody"];
        };
      };
    };
  };
  upsert_authenticated_user_setting_route: {
    parameters: {
      query?: never;
      header?: never;
      path?: never;
      cookie?: never;
    };
    requestBody: {
      content: {
        "application/json": components["schemas"]["AuthenticatedUpsertUserSettingRequest"];
      };
    };
    responses: {
      200: {
        headers: {
          [name: string]: unknown;
        };
        content: {
          "application/json": components["schemas"]["UpsertUserSettingResponse"];
        };
      };
      /** @description validation_failed */
      400: {
        headers: {
          [name: string]: unknown;
        };
        content: {
          "application/json": components["schemas"]["AccountFailureBody"];
        };
      };
      /** @description auth_required */
      401: {
        headers: {
          [name: string]: unknown;
        };
        content: {
          "application/json": components["schemas"]["AccountFailureBody"];
        };
      };
      /** @description setting_not_found */
      404: {
        headers: {
          [name: string]: unknown;
        };
        content: {
          "application/json": components["schemas"]["AccountFailureBody"];
        };
      };
    };
  };
  remove_authenticated_user_setting_route: {
    parameters: {
      query?: never;
      header?: never;
      path: {
        /** @description User setting key to remove */
        key: components["schemas"]["UserSettingKey"];
      };
      cookie?: never;
    };
    requestBody?: never;
    responses: {
      200: {
        headers: {
          [name: string]: unknown;
        };
        content: {
          "application/json": components["schemas"]["RemoveUserSettingResponse"];
        };
      };
      /** @description validation_failed */
      400: {
        headers: {
          [name: string]: unknown;
        };
        content: {
          "application/json": components["schemas"]["AccountFailureBody"];
        };
      };
      /** @description auth_required */
      401: {
        headers: {
          [name: string]: unknown;
        };
        content: {
          "application/json": components["schemas"]["AccountFailureBody"];
        };
      };
      /** @description setting_not_found */
      404: {
        headers: {
          [name: string]: unknown;
        };
        content: {
          "application/json": components["schemas"]["AccountFailureBody"];
        };
      };
    };
  };
  health_route: {
    parameters: {
      query?: never;
      header?: never;
      path?: never;
      cookie?: never;
    };
    requestBody?: never;
    responses: {
      /** @description Service is live */
      200: {
        headers: {
          [name: string]: unknown;
        };
        content: {
          "application/json": components["schemas"]["HealthResponse"];
        };
      };
    };
  };
  accept_invitation_route: {
    parameters: {
      query?: never;
      header?: never;
      path: {
        /** @description Opaque invitation token */
        token: string;
      };
      cookie?: never;
    };
    requestBody: {
      content: {
        "application/json": components["schemas"]["AcceptInvitationBody"];
      };
    };
    responses: {
      /** @description Invitation accepted */
      201: {
        headers: {
          [name: string]: unknown;
        };
        content: {
          "application/json": components["schemas"]["AcceptInvitationResponseCookie"];
        };
      };
      /** @description validation_failed */
      400: {
        headers: {
          [name: string]: unknown;
        };
        content: {
          "application/json": components["schemas"]["AccountFailureBody"];
        };
      };
      /** @description invitation_not_found */
      404: {
        headers: {
          [name: string]: unknown;
        };
        content: {
          "application/json": components["schemas"]["AccountFailureBody"];
        };
      };
      /** @description invitation_expired or invitation_already_consumed */
      410: {
        headers: {
          [name: string]: unknown;
        };
        content: {
          "application/json": components["schemas"]["AccountFailureBody"];
        };
      };
    };
  };
  sign_in_route: {
    parameters: {
      query?: never;
      header?: never;
      path?: never;
      cookie?: never;
    };
    requestBody: {
      content: {
        "application/json": components["schemas"]["SignInRequest"];
      };
    };
    responses: {
      /** @description Sign-in succeeded */
      200: {
        headers: {
          [name: string]: unknown;
        };
        content: {
          "application/json": components["schemas"]["SignInResponseCookie"];
        };
      };
      /** @description validation_failed */
      400: {
        headers: {
          [name: string]: unknown;
        };
        content: {
          "application/json": components["schemas"]["AccountFailureBody"];
        };
      };
      /** @description invalid_credential */
      401: {
        headers: {
          [name: string]: unknown;
        };
        content: {
          "application/json": components["schemas"]["AccountFailureBody"];
        };
      };
    };
  };
  revoke_route: {
    parameters: {
      query?: never;
      header?: never;
      path?: never;
      cookie?: never;
    };
    requestBody?: never;
    responses: {
      /** @description Session revoked */
      204: {
        headers: {
          [name: string]: unknown;
        };
        content?: never;
      };
    };
  };
}
