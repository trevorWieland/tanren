...
def parse_tanren_yml(project_root: Path) -> TanrenConfig | None:
    ...
    Returns:
        TanrenConfig if the file exists, None otherwise.
    """
...
def discover_env_vars_from_dotenv_example(project_root: Path) -> list[RequiredEnvVar]:
    ...
    Returns:
        List of RequiredEnvVar parsed from .env.example keys.
    """
...
def load_env_layers(
    ...
    Returns:
        Tuple of (merged_env, source_map) where source_map tracks each key's origin.
    """
...
def resolve_env_var(
    ...
    Returns:
        Tuple of (value, source), both None if the key is not found.
    """
    if key in merged:
