"""Queue-based run service — step-by-step provision/execute/teardown.

Each operation enqueues a step to the job queue. The worker processes
it and updates the state store. The API reads status from projections.
"""
...
class RunServiceV2:
    ...
    async def provision(self, body: object) -> RunEnvironment:
        ...
        req = body if isinstance(body, ProvisionRequest) else ProvisionRequest.model_validate(body)
        epoch = time.time_ns()
    ...
    async def execute(self, env_id: str, body: object = None) -> RunExecuteAccepted:  # noqa: ARG002
    ...
    async def full(self, body: object) -> DispatchAccepted:
        ...
        req = body if isinstance(body, RunFullRequest) else RunFullRequest.model_validate(body)
