"""Queue-based dispatch service — enqueue-only, reads from StateStore.

This replaces the in-process execution model in ``dispatch.py`` with a
queue-based approach: the API enqueues a provision step and the worker
processes it.
"""
