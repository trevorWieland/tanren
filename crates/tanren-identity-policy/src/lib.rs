...
// Domain ID newtypes live in the `ids` submodule so the crate stays
// under the workspace 500-line budget. Public API is re-exported here
// so downstream crates import from the same paths as before.
mod ids;
pub use ids::{AccountId, InvitationId, MembershipId, OrgId};
