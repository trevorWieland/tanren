...
mod tests {
    ...
    #[test]
    fn non_utf8_diff_is_summarized() {
        let diff = exact_content_diff(&[0xff, 0xfe, 0x01], &[0xff, 0xfe, 0x02], Path::new("x"));
        assert!(
            diff.unified_diff.contains("non-UTF8/binary"),
            "expected non-text summary, got: {}",
            diff.unified_diff
        );
    }

    #[test]
    fn oversized_text_diff_is_summarized() {
        let expected = "A".repeat(MAX_TEXT_BYTES + 1);
        let actual = "B".repeat(MAX_TEXT_BYTES + 1);
        let diff = exact_content_diff(expected.as_bytes(), actual.as_bytes(), Path::new("x"));
        assert!(
            diff.unified_diff.contains("text too large"),
            "expected oversized summary, got: {}",
            diff.unified_diff
        );
    }

    #[test]
    fn contextual_diff_truncates_large_change_windows() {
        let expected = (0..200).fold(String::new(), |mut out, i| {
            let _ = writeln!(&mut out, "old-{i}");
            out
        });
        let actual = (0..200).fold(String::new(), |mut out, i| {
            let _ = writeln!(&mut out, "new-{i}");
            out
        });
        let diff = exact_content_diff(expected.as_bytes(), actual.as_bytes(), Path::new("x"));
        assert!(
            diff.unified_diff.contains("truncated"),
            "expected truncation marker, got: {}",
            diff.unified_diff
        );
        assert!(
            diff.unified_diff.lines().count() < 180,
            "diff output should stay bounded"
        );
    }
}
