...
mod tests {
    ...
    #[test]
    fn manifest_contains_expected_files() {
        let files = generated_manifest_artifacts();
        assert_eq!(
            files,
            vec![
                "spec.md",
    ...
    #[test]
    fn audit_and_signposts_are_generated_readonly() {
        let readonly = readonly_protected_artifacts();
        assert!(readonly.contains(&"audit.md"));
        assert!(readonly.contains(&"signposts.md"));
    }
}
