...
impl TanrenMcp {
    ...
    #[rmcp::tool(
        ...
        description = "List organizations the account belongs to. Returns memberships and the active org. Failure codes match the shared taxonomy."
    )]
    async fn organization_list(
        &self,
        Parameters(req): Parameters<OrgAccountRequest>,
    ) -> Result<CallToolResult, McpError> {
        match self
            .handlers
            .list_organizations(self.store.as_ref(), req.account_id)
            .await
        {
            Ok(response) => Ok(success(&response)),
            Err(err) => Ok(map_failure(err)),
    ...
    #[rmcp::tool(
        ...
        description = "Switch the active organization for the given account. Failure code: organization-not-member."
    )]
    async fn organization_switch(
        &self,
        Parameters(req): Parameters<SwitchOrgRequest>,
    ) -> Result<CallToolResult, McpError> {
        match self
            .handlers
            .switch_active_org(
                self.store.as_ref(),
                req.account_id,
                SwitchActiveOrganizationRequest { org_id: req.org_id },
            )
            .await
        {
            Ok(response) => Ok(success(&response)),
    ...
    #[rmcp::tool(
        name = "organization.list_projects",
        description = "List projects for the account's currently active organization. Returns an empty list when no active org."
    )]
    async fn organization_list_projects(
        &self,
        Parameters(req): Parameters<OrgAccountRequest>,
    ) -> Result<CallToolResult, McpError> {
        match self
            .handlers
            .list_active_org_projects(self.store.as_ref(), req.account_id)
            .await
        {
            Ok(response) => Ok(success(&response)),
            Err(err) => Ok(map_failure(err)),
