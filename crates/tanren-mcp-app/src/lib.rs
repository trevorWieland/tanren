...
//!
//! The rmcp tool surface, API-key middleware, and host-header allowlist
//! live here so the BDD harness can exercise this code without spinning
//! up a child process.
...
impl TanrenMcp {
    ...
    async fn posture_set(
        ...
        match self
            .handlers
            .set_posture_raw(self.store.as_ref(), actor, &request.posture)
            .await
        {
            Ok(response) => Ok(success(&response)),
            Err(err) => Ok(map_failure(err)),
...
/// Build rmcp config honouring `TANREN_MCP_ALLOWED_HOSTS`.
