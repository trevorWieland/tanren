...
impl TanrenMcp {
    ...
    #[rmcp::tool(
        name = "deployment_posture.list",
        description = "List every supported deployment posture with capability availability summary."
    )]
    async fn deployment_posture_list(&self) -> Result<CallToolResult, McpError> {
        Ok(success(&self.handlers.list_supported_deployment_postures()))
    }

    #[rmcp::tool(
        name = "deployment_posture.get",
        description = "Read the current deployment posture for a scope. Input shape is tanren_contract::DeploymentPostureScope."
    )]
    ...
    #[rmcp::tool(
        name = "deployment_posture.set",
        description = "Set deployment posture for a scope. Input shape is tanren_contract::SetDeploymentPostureRequest and output shape is tanren_contract::SetDeploymentPostureResponse."
    )]
