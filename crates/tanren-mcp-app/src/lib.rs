...
use tanren_contract::{
    AcceptInvitationRequest, CreateOrganizationRequest, SignInRequest, SignUpRequest,
};
...
/// MCP tool surface. Delegates to the shared `Handlers` facade so all
/// interfaces resolve to the same logic.
#[derive(Clone)]
...
impl TanrenMcp {
    ...
    /// Create-organization tool. Mirrors `POST /organizations`.
    #[rmcp::tool(
        name = "create_organization",
        description = "Create a new Tanren organization. The authenticated caller becomes the bootstrap administrator. Failure codes: unauthenticated, duplicate_name."
    )]
    async fn create_organization(
        &self,
        Parameters(input): Parameters<CreateOrganizationInput>,
    ) -> Result<CallToolResult, McpError> {
        match self
            .handlers
            .create_organization(
                self.store.as_ref(),
                &input.session_token,
                CreateOrganizationRequest { name: input.name },
            )
            .await
        {
            Ok(response) => Ok(success(&response)),
    ...
    /// List-organizations tool. Mirrors `GET /organizations`.
    #[rmcp::tool(
        name = "list_organizations",
        description = "List organizations the authenticated caller is a member of. Failure codes: unauthenticated."
    )]
    async fn list_organizations(
        ...
        match self
            .handlers
            .list_organizations(self.store.as_ref(), &input.session_token)
            .await
        {
            Ok(response) => Ok(success(&response)),
            Err(err) => Ok(map_failure(err)),
    ...
    /// `tool_router` on rmcp macro versions that use the static builder.
...
/// MCP input for `create_organization`. Combines the session token with
...
/// MCP input for `list_organizations`.
#[derive(Debug, Clone, Serialize, Deserialize, schemars::JsonSchema)]
pub(crate) struct ListOrganizationsInput {
    pub session_token: SessionToken,
}

/// Encode a successful handler response as JSON `CallToolResult`.
fn success<T: Serialize>(value: &T) -> CallToolResult {
    let text = serde_json::to_string(value).unwrap_or_else(|_| "{}".to_owned());
    CallToolResult::success(vec![Content::text(text)])
}

/// Encode an [`AppServiceError`] as an MCP tool failure result.
...
/// Bootstrap API key sourced from `TANREN_MCP_API_KEY`.
#[derive(Debug, Clone)]
struct AuthConfig {
...
/// Build rmcp's `StreamableHttpServerConfig` honouring the env var.
...
/// Build the MCP axum router for the BDD wire-harness in
/// `tanren-testkit`.
