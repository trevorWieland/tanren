...
use tanren_contract::{
    AcceptInvitationRequest, AttentionSpecRequest, ListProjectsRequest, ProjectScopedViewsRequest,
    SignInRequest, SignUpRequest, SwitchProjectRequest,
};
...
impl TanrenMcp {
    ...
    async fn account_create(
        ...
        dispatch(self.handlers.sign_up(self.store.as_ref(), request)).await
    }
    ...
    async fn account_sign_in(
        ...
        dispatch(self.handlers.sign_in(self.store.as_ref(), request)).await
    }
    ...
    #[rmcp::tool(
        name = "project.list",
        description = "List all projects in the active account with per-project attention aggregation. Returns each project with specs needing attention."
    )]
    async fn project_list(
        ...
        dispatch(
            self.handlers
                .list_projects(self.store.as_ref(), req.account_id),
        )
        .await
    }

    #[rmcp::tool(
        name = "project.switch_active",
        description = "Switch the active project for the given account. Rejects projects outside the account. Returns the new project view and scoped data."
    )]
    async fn project_switch_active(
        &self,
        Parameters(req): Parameters<SwitchProjectRequest>,
    ) -> Result<CallToolResult, McpError> {
        dispatch(self.handlers.switch_active_project(
            self.store.as_ref(),
            req.account_id,
            req.project_id,
        ))
    ...
    #[rmcp::tool(
        name = "project.scoped_views",
        description = "Return specs, loops, and milestones for the currently active project. Requires an active project to be set."
    )]
    async fn project_scoped_views(
        &self,
        Parameters(req): Parameters<ProjectScopedViewsRequest>,
    ) -> Result<CallToolResult, McpError> {
        dispatch(
            self.handlers
                .project_scoped_views(self.store.as_ref(), req.account_id),
        )
        .await
    }

    #[rmcp::tool(
        name = "project.attention_spec",
        description = "Drill down into a specific attention-flagged spec. Returns spec id, name, and reason."
    )]
    async fn project_attention_spec(
        &self,
        Parameters(req): Parameters<AttentionSpecRequest>,
    ) -> Result<CallToolResult, McpError> {
        dispatch(self.handlers.attention_spec(
            self.store.as_ref(),
            req.account_id,
            req.project_id,
            req.spec_id,
...
async fn dispatch<F, T>(fut: F) -> Result<CallToolResult, McpError>
where
