...
//! This crate hosts the rmcp tool surface plus API-key auth middleware.
...
impl TanrenMcp {
    ...
    async fn account_my_permissions(
        ...
        let context = MyPermissionsContext {
            session_account_id,
            requested_account_id: request.target_account_id.unwrap_or(session_account_id),
        };
        match self
            .handlers
...
fn auth_required_failure(summary: &str) -> CallToolResult {
    let body = json!({
        "code": "auth_required",
        "summary": summary,
    });
