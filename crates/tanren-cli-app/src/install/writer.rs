...
fn write_manifest(plan: &InstallPlan, report: &mut InstallReport) -> Result<(), InstallError> {
    ...
    write_bytes(absolute, payload.as_bytes())?;
...
fn remove_file(removal: &PlannedRemoval) -> Result<(), InstallError> {
    fs::remove_file(removal.absolute_path()).map_err(|err| InstallError::RemoveFailure {
        path: removal.absolute_path().display().to_string(),
        message: err.to_string(),
    })
}

fn write_file(write: &PlannedWrite) -> Result<(), InstallError> {
    write_bytes(write.absolute_path(), write.content_bytes())
}

fn write_bytes(absolute: &Path, content: &[u8]) -> Result<(), InstallError> {
