...
use tanren_contract::{AccountFailureReason, UserConfigurationFailureReason};
...
pub(crate) fn map_app_error(err: AppServiceError) -> Response {
    match err {
        ...
        AppServiceError::Configuration(reason) => configuration_failure_body(&reason),
...
fn configuration_failure_body(reason: &UserConfigurationFailureReason) -> Response {
    let status =
        StatusCode::from_u16(reason.http_status()).unwrap_or(StatusCode::INTERNAL_SERVER_ERROR);
    (
        status,
