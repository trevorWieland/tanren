...
use super::web::WebHarness;
use super::{HarnessError, HarnessKind, HarnessResult};

pub struct ProjectWebHarness {
    inner: WebHarness,
}

impl std::fmt::Debug for ProjectWebHarness {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("ProjectWebHarness").finish_non_exhaustive()
    }
}

impl ProjectWebHarness {
    pub async fn spawn() -> HarnessResult<Self> {
        Ok(Self {
            inner: WebHarness::spawn().await?,
        })
    }
}
