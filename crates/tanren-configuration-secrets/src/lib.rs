...
impl SecretOwnerScope {
    /// Canonical storage/wire key for this owner scope.
    #[must_use]
...
impl std::fmt::Display for SecretOwnerScope {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
...
impl std::str::FromStr for SecretOwnerScope {
    ...
    fn from_str(raw: &str) -> Result<Self, Self::Err> {
        match raw.trim().to_ascii_lowercase().as_str() {
            ...
            other => Err(format!("unknown secret owner scope: {other}")),
        }
    }
}
...
impl SecretLifecycleStatus {
    /// Canonical storage/wire key for this lifecycle status.
    #[must_use]
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::Active => "active",
...
impl std::fmt::Display for SecretLifecycleStatus {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.write_str(self.as_str())
    }
}
...
impl std::str::FromStr for SecretLifecycleStatus {
    ...
    fn from_str(raw: &str) -> Result<Self, Self::Err> {
        match raw.trim().to_ascii_lowercase().as_str() {
            ...
            other => Err(format!("unknown secret lifecycle status: {other}")),
        }
    }
}
...
impl BaselineUsePolicy {
    /// Canonical storage/wire key for this use policy.
    #[must_use]
    pub const fn as_str(self) -> &'static str {
        match self {
...
impl std::fmt::Display for BaselineUsePolicy {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.write_str(self.as_str())
    }
}
...
impl std::str::FromStr for BaselineUsePolicy {
    ...
    fn from_str(raw: &str) -> Result<Self, Self::Err> {
        match raw.trim().to_ascii_lowercase().as_str() {
            ...
            other => Err(format!("unknown baseline use policy: {other}")),
        }
    }
}
