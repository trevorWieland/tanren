...
#[cfg(test)]
mod tests {
    use super::{SummaryRow, tuple_to_summary};
    use crate::entity::enums::{DispatchStatusModel, LaneModel, OutcomeModel};
    use chrono::Utc;
    use tanren_domain::{DispatchMode, DispatchStatus, Lane, Outcome};

    #[test]
    fn tuple_to_summary_builds_domain_summary_without_json() {
        let now = Utc::now();
        let row: SummaryRow = (
            uuid::Uuid::now_v7(),
            "manual".to_owned(),
            DispatchStatusModel::Running,
            Some(OutcomeModel::Success),
            LaneModel::Impl,
            "demo".to_owned(),
            now,
            now,
        );
        let summary = tuple_to_summary(row).expect("summary");
        assert_eq!(summary.mode, DispatchMode::Manual);
        assert_eq!(summary.status, DispatchStatus::Running);
        assert_eq!(summary.outcome, Some(Outcome::Success));
        assert_eq!(summary.lane, Lane::Impl);
        assert_eq!(summary.project.as_str(), "demo");
    }

    #[test]
    fn tuple_to_summary_rejects_empty_project() {
        let now = Utc::now();
        let row: SummaryRow = (
            uuid::Uuid::now_v7(),
            "manual".to_owned(),
            DispatchStatusModel::Pending,
            None,
            LaneModel::Impl,
            String::new(),
            now,
        ...
        let err = tuple_to_summary(row).expect_err("empty project");
        assert!(err.to_string().contains("invalid project column"));
    }
}
