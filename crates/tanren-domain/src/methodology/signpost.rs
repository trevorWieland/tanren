...
mod tests {
    ...
    #[test]
    fn status_is_open() {
        assert!(SignpostStatus::Unresolved.is_open());
        assert!(!SignpostStatus::Resolved.is_open());
        assert!(!SignpostStatus::Deferred.is_open());
        assert!(!SignpostStatus::ArchitecturalConstraint.is_open());
    }

    #[test]
    fn status_roundtrip() {
        for s in [
            SignpostStatus::Unresolved,
            SignpostStatus::Resolved,
            SignpostStatus::Deferred,
            SignpostStatus::ArchitecturalConstraint,
        ] {
            let json = serde_json::to_string(&s).expect("serialize");
            let back: SignpostStatus = serde_json::from_str(&json).expect("deserialize");
            assert_eq!(s, back);
        }
    }
}
