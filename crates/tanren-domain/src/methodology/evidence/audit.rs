...
mod tests {
    ...
    fn roundtrip_stable() {
        let a = AuditFrontmatter {
            ...
            fix_now_count: 0,
            rubric: vec![],
            non_negotiables_compliance: vec![],
            findings: vec![],
            generated_at: Utc::now(),
        };
        let doc = a.render_to_markdown("audit body\n").expect("render");
        let (back, body) = AuditFrontmatter::parse_from_markdown(&doc).expect("parse");
        assert_eq!(back, a);
        assert_eq!(body, "audit body\n");
    }
}
