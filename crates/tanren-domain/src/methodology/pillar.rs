...
mod tests {
    ...
    #[test]
    fn thirteen_builtin_pillars() {
        assert_eq!(builtin_pillars().len(), 13);
    }

    #[test]
    fn builtin_ids_are_unique() {
        let mut ids: Vec<String> = builtin_pillars()
            .into_iter()
        ...
        let before = ids.len();
        ids.dedup();
        assert_eq!(ids.len(), before, "pillar ids must be unique");
    }

    #[test]
    fn score_range_enforced() {
        assert!(PillarScore::try_new(0).is_err());
        assert!(PillarScore::try_new(11).is_err());
        assert_eq!(PillarScore::try_new(1).expect("ok").get(), 1);
        assert_eq!(PillarScore::try_new(10).expect("ok").get(), 10);
    }

    #[test]
    fn score_deserialize_rejects_out_of_range() {
        assert!(serde_json::from_str::<PillarScore>("0").is_err());
        assert!(serde_json::from_str::<PillarScore>("11").is_err());
        let ok: PillarScore = serde_json::from_str("7").expect("ok");
        assert_eq!(ok.get(), 7);
    }

    #[test]
    fn applicable_at_includes_scope() {
        assert!(ApplicableAt::Both.includes(PillarScope::Task));
        assert!(ApplicableAt::Both.includes(PillarScope::Spec));
        assert!(ApplicableAt::TaskOnly.includes(PillarScope::Task));
        assert!(!ApplicableAt::TaskOnly.includes(PillarScope::Spec));
        assert!(ApplicableAt::SpecOnly.includes(PillarScope::Spec));
        assert!(!ApplicableAt::SpecOnly.includes(PillarScope::Task));
    }

    #[test]
    fn pillar_id_rejects_empty() {
        assert!(PillarId::try_new("").is_err());
        assert!(PillarId::try_new("   ").is_err());
    }
}
