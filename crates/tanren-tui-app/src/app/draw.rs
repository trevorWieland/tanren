...
/// Dispatch rendering based on the current screen variant.
pub(super) fn draw_screen(frame: &mut ratatui::Frame<'_>, screen: &Screen) {
    let area = frame.area();
    match screen {
        Screen::Menu { selected } => draw_menu(frame, area, *selected),
        Screen::SignUp(state) => draw_form(frame, area, "Sign up", state),
        Screen::SignIn(state) => draw_form(frame, area, "Sign in", state),
        Screen::AcceptInvitation(state) => {
            draw_form(frame, area, "Accept invitation", state);
        }
        Screen::CreateOrganization(state) => {
            draw_form(frame, area, "Create organization", state);
        }
        Screen::ListOrganizations(state) => {
            draw_form(frame, area, "List organizations", state);
        }
        Screen::CheckOrganizationPermission(state) => {
            draw_form(frame, area, "Check org permission", state);
        }
        Screen::ListOrganizationMembers(state) => {
            draw_form(frame, area, "List org members", state);
        }
        Screen::Outcome(view) => draw_outcome(frame, area, view),
    }
}
