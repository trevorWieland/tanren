...
class TestGitWorktreeManager:
    @pytest.mark.asyncio
    @patch("worker_manager.adapters.git_worktree.create_worktree", new_callable=AsyncMock)
    ...
    @pytest.mark.asyncio
    @patch("worker_manager.adapters.git_worktree.register_worktree", new_callable=AsyncMock)
    ...
    @pytest.mark.asyncio
    @patch("worker_manager.adapters.git_worktree.cleanup_worktree", new_callable=AsyncMock)
...
class TestGitPreflightRunner:
    @pytest.mark.asyncio
    @patch("worker_manager.adapters.git_preflight.run_preflight", new_callable=AsyncMock)
...
class TestGitPostflightRunner:
    @pytest.mark.asyncio
    @patch("worker_manager.adapters.git_postflight.run_postflight", new_callable=AsyncMock)
    async def test_run_delegates(self, mock_run):
...
class TestSubprocessSpawner:
    @pytest.mark.asyncio
    @patch("worker_manager.adapters.subprocess_spawner.spawn_process", new_callable=AsyncMock)
...
class TestDotenvEnvValidator:
    @pytest.mark.asyncio
    @patch("worker_manager.adapters.dotenv_validator.load_and_validate_env", new_callable=AsyncMock)
...
class TestDotenvEnvProvisioner:
    @patch("worker_manager.adapters.dotenv_provisioner.provision_worktree_env")
