...
Feature: Uninstall Tanren assets without deleting user work
  ...
  Rule: CLI uninstall preview, apply, and no-op witnesses

    @positive @cli
    Scenario: CLI uninstall apply removes generated assets while preserving user-owned files
    ...
    @falsification @cli
    Scenario: CLI repo-local uninstall confirm without a prior install does not exercise hosted account or project history deletion
      Given a clean repository fixture
