"""Tests for SqliteStore — EventStore, JobQueue, and StateStore protocols."""
...
class TestEventStoreAppend:
    async def test_append_and_query(self, store: SqliteStore) -> None:
    ...
    async def test_query_with_time_range(self, store: SqliteStore) -> None:
        for i in range(3):
            event = DispatchCreated(
    ...
    async def test_query_pagination(self, store: SqliteStore) -> None:
        for i in range(5):
            event = DispatchCreated(
...
class TestJobQueueEnqueueDequeue:
    async def test_enqueue_and_dequeue(self, store: SqliteStore) -> None:
        dispatch = _make_dispatch()
    ...
    async def test_dequeue_empty(self, store: SqliteStore) -> None:
        step = await store.dequeue(lane=Lane.IMPL, worker_id="w1", max_concurrent=1)
        assert step is None

    async def test_dequeue_respects_lane(self, store: SqliteStore) -> None:
        dispatch = _make_dispatch()
        await store.create_dispatch_projection(
            dispatch_id="wf-test-1-100",
            mode=DispatchMode.AUTO,
...
class TestJobQueueConcurrency:
    async def test_dequeue_respects_max_concurrent(self, store: SqliteStore) -> None:
        dispatch = _make_dispatch()
        await store.create_dispatch_projection(
            dispatch_id="wf-test-1-100",
            mode=DispatchMode.AUTO,
...
class TestJobQueueAckNack:
    async def test_ack_stores_result(self, store: SqliteStore) -> None:
        dispatch = _make_dispatch()
        await store.create_dispatch_projection(
            dispatch_id="wf-test-1-100",
            mode=DispatchMode.AUTO,
    ...
    async def test_nack_marks_failed(self, store: SqliteStore) -> None:
        dispatch = _make_dispatch()
        await store.create_dispatch_projection(
            dispatch_id="wf-test-1-100",
            mode=DispatchMode.AUTO,
    ...
    async def test_nack_with_retry(self, store: SqliteStore) -> None:
        dispatch = _make_dispatch()
        await store.create_dispatch_projection(
            dispatch_id="wf-test-1-100",
            mode=DispatchMode.AUTO,
...
class TestStateStoreDispatches:
    async def test_create_and_get_dispatch(self, store: SqliteStore) -> None:
        dispatch = _make_dispatch()
        await store.create_dispatch_projection(
            dispatch_id="wf-test-1-100",
            mode=DispatchMode.AUTO,
    ...
    async def test_get_nonexistent_dispatch(self, store: SqliteStore) -> None:
        view = await store.get_dispatch("nonexistent")
        assert view is None

    async def test_update_dispatch_status(self, store: SqliteStore) -> None:
        dispatch = _make_dispatch()
        await store.create_dispatch_projection(
            dispatch_id="wf-test-1-100",
            mode=DispatchMode.AUTO,
    ...
    async def test_query_dispatches_by_status(self, store: SqliteStore) -> None:
        for i in range(3):
...
class TestStateStoreSteps:
    async def test_get_steps_for_dispatch(self, store: SqliteStore) -> None:
        dispatch = _make_dispatch()
        await store.create_dispatch_projection(
            dispatch_id="wf-test-1-100",
            mode=DispatchMode.AUTO,
    ...
    async def test_count_running_steps(self, store: SqliteStore) -> None:
        dispatch = _make_dispatch()
        await store.create_dispatch_projection(
            dispatch_id="wf-test-1-100",
            mode=DispatchMode.AUTO,
...
class TestStoreLifecycle:
    async def test_close(self, store: SqliteStore) -> None:
        await store.close()
        # Double close should be safe
        await store.close()

    async def test_full_dispatch_lifecycle(self, store: SqliteStore) -> None:
